...
This Version makes several changes from the base Surface graphics notebook.  
First, is simplifies and standardizes Station names.  Second, it facet graphics
for six variables and it adds a temperature by dissolved oxygen graphic.  We
also explore ways to integrate transformed chlorophyll axes into facet plots or
other multi-plot layouts.
...
much the same thing.  The logic used here can be generalized to more complex 
models and various marginal (adjusted) means.
...
`station`, `station_name`, `value`, and `bottom_flag` fields in the source data
frame. 
...
```{r test_jitter_plot,  fig.width = 3.5, fig.height = 3.5}
tmp <- nested_data %>%
...
```{r test_violin, fig.width = 3.5, fig.height = 3.5}
tmp <- nested_data %>%
  filter(parameter =='chl')
dat <- tmp$data[[1]]
...
## All Jitter Plots
```{r all_jitters, fig.width = 3.5, fig.height = 3.5}
...
## All Violin Plots
```{r all_violins, fig.width = 3.5, fig.height = 3.5}
...
## Draft Jitter Plot
```{r draft_facet_jitter, fig.width = 7, fig.height = 6}
...
are.  Luckily, in our use case, we DO know that the upper axis limit for 
...
so the labels we want

##### Evenly Spaced Tick Marks
```{r}
round(exp(seq(0, 5, 1)) + 1, 1)
```
Those are fairly unpleasant.  The alternative is to work things the other way,
and start with "clean" breaks, and work backwards.
...
## Revised Jitter Plot
```{r revised_facet_jitter_alt, fig.width = 7, fig.height = 7}
p <- ggplot(data_sel, aes(x = station_name, y = value)) +
...
## Violin Chart
```{r revised_facet_violin, fig.width = 7, fig.height = 6}
...
```{r revised_facet_bar, fig.width = 7, fig.height = 6}
p <- ggplot(data_sel, aes(x = station_name, y = value)) +
